    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'FachriBotz 𝗍𝗂𝖽𝖺𝗄 𝖺𝗄𝗍𝗂𝖿' : banned ? 'kamu dibanned' : 'Ada Apa ya tag-tag saia ?\nApakah Ada Bansos ?',
                'FachriBotz By Fachri',
...
╠═〘 FachriBotz 〙 ═`.trim(), 'FachriBotz By Fachri', 'Pemilik\nBot', '#owner', 'Ga jadi deh..', 'gajadi', m)
}
...
if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`⏲️ 𝖱𝗎𝗇𝗍𝗂𝗆𝖾: ${uptime} | 🛰 𝖬𝗈𝖽𝖾: ${global.opts['self'] ? '𝖯𝗋𝗂𝗏𝖺𝗍𝖾' : setting.groupOnly ? 'Hanya Grup' : '𝖯𝗎𝖻𝗅𝗂𝖼'} | FachriBotz By Fachri`).catch(_ => _)
